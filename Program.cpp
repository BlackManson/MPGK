...
GLint ProgramMPGK::obrotShader;
GLint theta;
float ProgramMPGK::ratio = 0.0f;
...
void ProgramMPGK::stworzenieOkna(int argc, char** argv)
{
    ...
    glutCreateWindow("Program Lukasz and Patrycja");
}
...
void  ProgramMPGK::wyswietl()
{
    ...
    glUniform1f(obrotShader, sin(zmiana));
    glUniform1f(zmiennaShader, zmiana);
    ...
    glDrawElements(GL_TRIANGLES, iloscWie, GL_UNSIGNED_INT, 0);
...
{
    const int iloscWie = 36;
    Wektor<4> Wektory[iloscWie] = {
            Wektor<4>(-0.4f, 0.4f, 0.4f, 1.0f),
            Wektor<4>(-0.4f, 0.4f, -0.4f, 1.0f),
            Wektor<4>(-0.4f,  -0.4f, 0.4f, 1.0f),
            Wektor<4>(-0.4f,  -0.4f, -0.4f, 1.0f),
    ...
    static int iloscKol = 6;
    Wektor<4> Kolory[iloscWie] = {
            Wektor<4>(0.0f,0.0f, 1.0f, 1.0f),
            Wektor<4>(0.0f,0.0f, 1.0f, 1.0f),
            Wektor<4>(0.0f,0.0f, 1.0f, 1.0f),
            Wektor<4>(0.0f,0.0f, 1.0f, 1.0f),

            Wektor<4>(1.0f,1.0f, 1.0f, 1.0f),
            Wektor<4>(1.0f,1.0f, 1.0f, 1.0f),
            Wektor<4>(1.0f,1.0f, 1.0f, 1.0f),
            Wektor<4>(1.0f,1.0f, 1.0f, 1.0f),
            ...
            Wektor<4>(1.0f,0.0f, 1.0f, 1.0f),
            Wektor<4>(1.0f,0.0f, 1.0f, 1.0f),
            Wektor<4>(1.0f,0.0f, 1.0f, 1.0f),
            Wektor<4>(1.0f,0.0f, 1.0f, 1.0f),

            Wektor<4>(1.0f,1.0f, 0.0f, 1.0f),
            Wektor<4>(1.0f,1.0f, 0.0f, 1.0f),
            Wektor<4>(1.0f,1.0f, 0.0f, 1.0f),
            Wektor<4>(1.0f,1.0f, 0.0f, 1.0f),
    };

    GLfloat wierzcholki[iloscWie*4*2];
    GLint licznik = 0;
    for (int i = 0; i < iloscWie; i++) {
        wierzcholki[licznik] = Wektory[i].getX();
        licznik += 1;
        wierzcholki[licznik] = Wektory[i].getY();
        licznik += 1;
        wierzcholki[licznik] = Wektory[i].getZ();
        licznik += 1;
        wierzcholki[licznik] = Wektory[i].getW();
        licznik += 1;
        wierzcholki[licznik] = Kolory[i].getX();
        licznik += 1;
        wierzcholki[licznik] = Kolory[i].getY();
        licznik += 1;
        wierzcholki[licznik] = Kolory[i].getZ();
        licznik += 1;
        wierzcholki[licznik] = Kolory[i].getW();
        licznik += 1;
    }
...
void ProgramMPGK::stworzenieProgramu()
{
    ...
    const char* vertexShader =
			...
			void main()			 \n																	\
			{		 \n																				\
				gl_Position = vec4(obrotShader * polozenie.x*cos(zmianaShader)-(obrotShader * polozenie.z)*sin(zmianaShader), (obrotShader * polozenie.y)*cos(zmianaShader)-(obrotShader * polozenie.z)*sin(zmianaShader), (obrotShader * polozenie.y)*sin(zmianaShader)+(obrotShader * polozenie.z)*cos(zmianaShader), polozenie.w); \n		\
				kolorFS = kolorVS; \n																\
			}";
    ...
    obrotShader = glGetUniformLocation(programZShaderami, "obrotShader");
    if (zmiennaShader == -1)
    {
